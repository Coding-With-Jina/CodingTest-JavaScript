...
const cityCnt = Number(input.shift());
const [distances, price] = [input.shift().split(' ').map(Number), input.map(line => line.split(' ').map(Number)).flat()];
  
//console.log(distances); // [2, 3, 1]
//console.log(price); // [5, 2, 4, 1]
  
//총 필요한 거리를 파악하고
//기름값이 가까운 주유소를 찾는다
//기름값이 가까운 주유소에서 남은 거리의 모든 기름을 다 채워넣는다 
