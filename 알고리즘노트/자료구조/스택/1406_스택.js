...
const lStack = input.shift().split(""); //초기 편집 문자열
const commandCnt = Number(input.shift()); //입력할 명령어 갯수
const rStack = [];
...
for(let i=0;i<commandCnt;i++){
    ...
    if(cmd==='L' && lStack.length){//커서를 왼쪽으로 한 칸 옮김 (커서가 문장의 맨 앞이면 무시됨)
        rStack.push(lStack.pop());
    }
    else if(cmd==='D'&& rStack.length){//커서를 오른쪽으로 한 칸 옮김 (커서가 문장의 맨 뒤이면 무시됨)
    ...
    else if(cmd==='P') {//$라는 문자를 커서 왼쪽에 추가함
        lStack.push(value);
    }
    //console.log(lStack);
}
